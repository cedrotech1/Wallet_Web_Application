...
function CreateTransaction({ addTransaction }: Props) {
    const [loading, setLoading] = useState(false)
    const [subcategories, setSubcategories] = useState<any[]>([])
    const [categories, setCategories] = useState<any[]>([])
    const [accounts, setAccounts] = useState<any[]>([])
    const { register, handleSubmit, formState: { errors }, reset, watch } = useForm<createTransactionI>({ mode: 'onBlur' })
        ...
        <div>
            <form onSubmit={handleSubmit(createTransaction)}>
                ...
                <div>
                    ...
                    <select disabled={category == ""} {...register('subcategoryId')} className='input' >
                        <option value=''> select category</option>
                        {subcategories && subcategories.length !== 0 && subcategories.filter((el) => el.categoryId == category).map((el) => (
                            <option value={el.id}>{el.name}</option>
                        ))}
                    </select>
                </div>
                <div>
                    ...
                    <select  {...register('accountId')} className='input' >
                        <option value=''> select category</option>
                        {accounts && accounts.length !== 0 && accounts.map((el) => (
                            <option value={el.id}> {el.name}</option>
                        ))}
                    </select>
                </div>
                <div>
                    <label>Amount</label>
                    <input {...register('amount')} min='0' step="0.001" type='number' className='input' />
                </div>
                <button type='submit' className='w-full  mt-3 h-10 bg-black text-white rounded-[8px]'>
                    {loading ? <HashLoader color='#FFF' loading={loading} size={15} /> : 'Add transaction'}
                </button>
